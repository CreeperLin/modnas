...
class Slot(nn.Module):
    ...
    def __init__(self, chn_in, chn_out, stride, name=None, pid=None):
        super().__init__()
        ...
        self.stride = stride
        self.ent = None
        print('slot {} ({} {}) {}: declared {} {} {}'.format(self.sid, self.param_pid, self.pid, self.name, self.chn_in, self.chn_out, self.stride))
    ...
    def set_entity(self, ent):
        self.ent = ent
        print('slot {} ({}) {}: set to {}'.format(self.sid, self.pid, self.name, ent.__class__.__name__))
...
def default_predefined_converter(slot, mixop_cls, *args, **kwargs):
    ent = mixop_cls(slot.chn_in, slot.stride, *args, **kwargs)
    return ent
...
def convert_from_predefined_net(model, convert_fn, *args, **kwargs):
...
def convert_from_genotype(model, genotype, convert_fn=None, *args, **kwargs):
    ...
    print('building net from genotype: {}'.format(genotype))
    try:
