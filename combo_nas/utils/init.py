...
class DefaultModelInitializer():
    ...
    def init_tensor(self, init_type, t, gain, fan_in, fan_out):
        if not init_type in _initializers or t is None:
            return
    ...
    def __call__(self, model):
        ...
        for m in model.modules():
            if isinstance(m, nn.Conv2d):
                if self.conv_init_type is None: continue
                rec_size = m.kernel_size[0] * m.kernel_size[1]
                fan_in, fan_out = rec_size * m.in_channels, rec_size * m.out_channels
                ...
                if not self.bn_momentum is None:
                    m.momentum = self.bn_momentum
                if not self.bn_eps is None:
                    m.eps = self.bn_eps
                if not m.affine: continue
                fan_in = fan_out = m.num_features
                self.init_tensor(self.bn_init_type, m.weight, gain, fan_in, fan_out)
                self.init_tensor(self.bias_init_type, m.bias, gain, fan_in, fan_out)
            elif isinstance(m, nn.Linear):
                if self.fc_init_type is None: continue
